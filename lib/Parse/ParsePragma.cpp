...
void Parser::resetPragmaHandlers() {
  ...
  // TASKIFY
  PP.RemovePragmaHandler(TaskifyHandler.get());
  TaskifyHandler.reset();
}
...
/// TASKIFY:
/// HandlePragmaTaskify - Handle \#pragma taskify.
///
enum taskify_operation {
	out,
	finest,
	other,
	...
	error // used for error token names
};

taskify_operation getTaskifyOperation(std::string const& tokenOp) {
	if (tokenOp == "out") return out;
	if (tokenOp == "finest") return finest;
	if (tokenOp == "=") return other;

	if (tokenOp == "XData") return XData;
	if (tokenOp == ",") return comma;
	if (tokenOp == "&") return ampersend;
...
bool TaskifyAlgorithmBody(Preprocessor &PP, Token &FirstToken, ASTContext::TaskifyStruct &taskStruct, ASTContext &context){
	...
	std::string tokName = "";
	while (FirstToken.isNot(tok::r_paren))
	{
		paramCounter++;
		tokName = PP.getSpelling(FirstToken);

		if (getTaskifyOperation(tokName) != XData){
		...
		if (getTaskifyOperation(tokName) != ampersend){
			PP.Diag(FirstToken.getLocation(), diag::err_function_declared_typedef);
			return false;
		}

		//read identifier and possible comma
		PP.Lex(FirstToken);
		parameters += PP.getSpelling(FirstToken);

		PP.Lex(FirstToken);
		tokName = PP.getSpelling(FirstToken);
		parameters += tokName + " ";

		if (getTaskifyOperation(tokName) == comma)
			PP.Lex(FirstToken);
	}
...
void PragmaTaskifyHandler::HandlePragma(Preprocessor &PP, PragmaIntroducerKind Introducer, Token &FirstToken)
{
	...
	while (FirstToken.isNot(clang::tok::eod))   // check if it's the end of the line
	{
		// get next token
		PP.Lex(FirstToken);
		std::string tokenName = PP.getSpelling(FirstToken);

		switch (getTaskifyOperation(tokenName))
		{
		case out:
		{
			// read two tokens after this to get the name of output file
			PP.Lex(FirstToken);
			PP.Lex(FirstToken);
		...
		case finest:
		{
			// read two tokens after this to get the name of finest function
			PP.Lex(FirstToken);
			PP.Lex(FirstToken);
		...
		// here we handle invalid arguments
		case error:
			//PP.Diag(FirstToken.getLocation(), diag::err_pragma_invalid_keyword) << "taskify out/finest expected";
			break;

			// here we handle the both the '=' and what is after
		case other:
		default:
			break;
		}
